...
const SC_GRABBER_SCRIPT: &str = include_str!("sc-grabber.ps1");
const HM_MONITORS_SCRIPT: &str = include_str!("hm-monitors.ps1");

fn get_powershell_exe() -> PathBuf {
    PathBuf::from("powershell.exe")
}

pub fn get_monitor_count() -> Result<u32> {
    let output = Command::new(get_powershell_exe())
        .arg("-Command") 
        .arg(HM_MONITORS_SCRIPT)
        .stdin(std::process::Stdio::null())
        .output()
        .context("Failed to execute hm-monitors.ps1 script")?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        error!(
            "hm-monitors.ps1 failed with status {:?}: {}",
            output.status.code(),
            stderr
        );
        anyhow::bail!("hm-monitors.ps1 script failed.");
    }
...
pub fn run(rx: Receiver<MonitorEvent>, paths: &AppPaths) -> Result<()> {
    ...
    run_command(
        &get_powershell_exe(),
        &["-Command", SC_GRABBER_SCRIPT],
        None,
        None,
    )
    .context("Failed to run sc-grabber.ps1")?;
        ...
        .context("Timeout or error waiting for screenshots from monitor thread")?
    {
        ...
        MonitorEvent::Error(e) => {
            error!("Monitor thread reported error during screenshot wait: {}", e);
            anyhow::bail!("Monitor thread error: {}", e);
        },
        ev => anyhow::bail!("Received unexpected event while waiting for screenshots: {:?}", ev),
    }

    if !paths.squiggle_bin.exists() {
         error!("Squiggle binary (.exe) not found at {}", paths.squiggle_bin.display());
    ...
    run_command(
        &paths.squiggle_bin, 
        &squiggle_args_os.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
        None,
        None,
    )
        ...
        .context("Timeout or error waiting for squiggle output from monitor thread")?
    {
        ...
        ev => anyhow::bail!("Received unexpected event while waiting for squiggle output: {:?}", ev),
    }

    if !paths.electron_exe.exists() {
         error!("SpatialShot UI executable (.exe) not found at {}", paths.electron_exe.display());
        anyhow::bail!("SpatialShot UI executable not found.");
    }

    let output_path_str = final_output_path.to_str()
