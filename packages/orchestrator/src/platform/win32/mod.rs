...
const GRABBER_SCRIPT: &str = include_str!("sc-grapper.ps1");
const MONITOR_SCRIPT: &str = include_str!("hm-monitors.ps1");

/// The main sequence for Windows.
pub fn run(rx: Receiver<MonitorEvent>, paths: &ProjectPaths) -> Result<()> {
    // Step 1: Run Screen Grabber
    println!("[SEQ] Running sc-grapper.ps1...");
    run_command(
        Command::new("powershell.exe")
            .arg("-ExecutionPolicy").arg("Bypass")
            .arg("-Command").arg(GRABBER_SCRIPT)
    ).context("Failed to execute sc-grapper.ps1.")?;
    ...
    // On Windows, `npm` is often `npm.cmd`, so we use cmd /c to be safe.
    let mut ui_cmd = Command::new("cmd.exe");
...
pub fn get_monitor_count() -> Result<u32> {
    let output = Command::new("powershell.exe").arg("-Command").arg(MONITOR_SCRIPT).output()?;
